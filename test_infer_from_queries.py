import pytest
import numpy as np
from infer_from_queries import infer_probas_for_multiple_queries
...
def test_infer_probas_for_multiple_queries(g, cid, sampling_method):
    n_queries = 20
    obs, c = gen_input(g)
    remaining_nodes = list(set(np.arange(g.num_vertices())) - set(obs))
    queries = np.random.permutation(remaining_nodes)[:n_queries]

    inf_proba_list, sampler, estimator = infer_probas_for_multiple_queries(
        g, obs, c, queries,
